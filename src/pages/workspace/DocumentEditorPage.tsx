...
;(function injectStyles() {
  if (typeof document === 'undefined' || document.getElementById('df-editor-styles')) return
...
const LEXICAL_THEME = {
  ...
  quote: 'df-quote',
  list: { ul: 'df-ul', ol: 'df-ol', listitem: 'df-li', nested: { listitem: 'df-li' } },
  link: 'df-link',
  code: 'df-code-block',
  text: {
    bold: 'df-bold', italic: 'df-italic',
    strikethrough: 'df-strike', underline: 'df-underline', code: 'df-inline-code',
  },
}
...
const CollaboratorAvatar = memo(({ user, index }: { user: ConnectedUser; index: number }) => {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        marginLeft: index === 0 ? 0 : -OVERLAP, zIndex: MAX_VISIBLE - index,
        ['--ring-color' as any]: ringColor, ['--ring-glow' as any]: glowColor,
    ...
    >
      {user.avatar_url && !imgFailed ? (
        ...
        <div className="w-full h-full rounded-full flex items-center justify-center text-[9px] font-bold select-none"
          style={{ background: 'oklch(0.38 0.16 285)', color: 'oklch(0.88 0.08 285)' }}>
...
function CollaboratorAvatars({ provider }: { provider: WebsocketProvider | null }) {
  ...
  return (
    <div className="flex items-center">
      {visible.map((u, i) => <CollaboratorAvatar key={u.clientId} user={u} index={i} />)}
      {overflow > 0 && (
        <div title={`${overflow} more`}
          className="flex items-center justify-center rounded-full text-[9px] font-bold select-none"
          style={{
            width: AVATAR_SIZE, height: AVATAR_SIZE,
            marginLeft: -OVERLAP, zIndex: 0, flexShrink: 0,
  ...
  return (
    <motion.div key={status}
      ...
      <span className="w-1.5 h-1.5 rounded-full shrink-0"
        style={{ background: cfg.dot, animation: status === 'connecting' ? 'pulse 1.2s infinite' : undefined }} />
      {cfg.label}
    </motion.div>
  )
}
...
function ToolbarPlugin() {
  ...
  function Btn({ onClick, icon: Icon, label }: { onClick: () => void; icon: React.FC<any>; label: string }) {
    return (
      <motion.button
        ...
        onMouseEnter={e => Object.assign(e.currentTarget.style, { background: 'rgba(255,255,255,0.07)', color: 'rgba(255,255,255,0.8)' })}
        onMouseLeave={e => Object.assign(e.currentTarget.style, { background: 'transparent', color: 'rgba(255,255,255,0.45)' })}
      >
  ...
  return (
    ...
    >
      <Btn icon={Heading1} label="H1" onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, 'h1' as any)} />
      <Btn icon={Heading2} label="H2" onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, 'h2' as any)} />
      <Btn icon={Heading3} label="H3" onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, 'h3' as any)} />
      <Sep />
      <Btn icon={Bold}          label="Bold"          onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, 'bold')} />
      <Btn icon={Italic}        label="Italic"        onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, 'italic')} />
      <Btn icon={Strikethrough} label="Strikethrough" onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, 'strikethrough')} />
      <Btn icon={Code}          label="Inline code"   onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, 'code')} />
      <Sep />
      <Btn icon={List}        label="Bullet list"   onClick={() => editor.dispatchCommand(INSERT_UNORDERED_LIST_COMMAND, undefined)} />
      <Btn icon={ListOrdered} label="Numbered list" onClick={() => editor.dispatchCommand(INSERT_ORDERED_LIST_COMMAND, undefined)} />
      <Btn icon={Quote}       label="Quote"         onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, 'quote' as any)} />
    </div>
  )
}
...
function TypingIndicatorPlugin({ provider }: { provider: WebsocketProvider }) {
  ...
  useEffect(() => {
    ...
    return editor.registerUpdateListener(({ tags }) => {
      ...
      clearTimeout(timer)
      timer = setTimeout(() => provider.awareness.setLocalStateField('typing', false), 1500)
    })
  }, [editor, provider])
  return null
}
...
const EditorTopbar = memo(({ cardTitle, wsStatus, onBack, provider }: {
  cardTitle: string
  wsStatus: WsStatus
  onBack: () => void
  ...
  >
    <div className="flex items-center gap-3 min-w-0">
      <motion.button onClick={onBack} aria-label="Back to board"
      ...
      <div className="flex flex-col min-w-0">
        <h1 className="font-bold text-[15px] tracking-tight truncate leading-tight"
          style={{ color: 'oklch(0.94 0.008 265)', fontFamily: 'var(--df-font-display)' }}>
          {cardTitle}
        </h1>
        <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.28)' }}>Document</p>
      </div>
    </div>

    <div className="flex items-center gap-3 shrink-0">
      <CollaboratorAvatars provider={provider} />
      {provider && <div className="w-px h-5" style={{ background: 'rgba(255,255,255,0.08)' }} />}
  ...
  return (
    <div className="max-w-3xl mx-auto px-8 py-12 space-y-3">
      {[0.55, 1, 0.83, 0.91, 0.67, 0.78, 0.6].map((w, i) => (
        <motion.div key={i} className="h-4 rounded-lg"
...
function ErrorScreen({ icon: Icon, message, onBack }: {
  icon: React.FC<any>; message: string; onBack: () => void
}) {
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
      ...
      <p className="font-semibold text-[15px]" style={{ color: 'oklch(0.88 0.012 265)' }}>{message}</p>
      <motion.button onClick={onBack} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
        className="text-[13px] font-bold px-4 py-2 rounded-xl"
        style={{ background: 'rgba(0,218,243,0.08)', border: '1px solid rgba(0,218,243,0.18)', color: 'oklch(0.82 0.14 198)' }}>
        Back to board
      </motion.button>
    </motion.div>
  )
      ...
      provider.awareness.on('change', () => {
        ...
        provider.awareness.getStates().forEach((state, clientId) => {
          if (clientId !== myClientId) {
            ...
            // Notify topbar subscribers
            awarenessProvider.awareness.emit('change', [{ added: [], updated: [clientId], removed: [] }, null])
          }
        })
      })
            ...
            <RichTextPlugin
              contentEditable={
                <ContentEditable
                  ...
                  style={{ fontFamily: 'var(--df-font-body,"Geist Variable",system-ui,sans-serif)' }}
              ...
              placeholder={
                <div
                  ...
                  style={{ color: 'rgba(255,255,255,0.16)', fontFamily: 'var(--df-font-body)' }}
                  aria-hidden>
...
export function DocumentEditorPage() {
  const { workspaceId, boardId, cardId } = useParams<{
    workspaceId: string; boardId: string; cardId: string
  }>()
  ...
  const { data: snapshotData, isLoading: isSnapshotLoading, isError: isSnapshotError                       } = useDocumentSnapshot({ documentId, tokenReady: Boolean(tokenData) })
  ...
  const [awarenessProvider, setAwarenessProvider] = useState<WebsocketProvider | null>(null)
  ...
  useEffect(() => {
    ...
    const awarenessPayload: AwarenessUser = { name: userName, color: cursorColor, avatar_url: avatarUrl }
  ...
  useEffect(() => {
    ...
    awarenessProvider.awareness.setLocalStateField('user', {
      name: userName, color: cursorColor, avatar_url: avatarUrl,
    })
  ...
  if (isTokenError)
    return <ErrorScreen icon={AlertCircle}
  ...
  if (isSnapshotError || isBoardError || (!documentId && !isBoardLoading && board))
    return <ErrorScreen icon={WifiOff} message="Failed to load document or board not found" onBack={handleBack} />

  const isBootstrapping =
    isBoardLoading || isTokenLoading || isSnapshotLoading ||
    !awarenessProvider || !board || !documentId || !tokenData

  return (
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: 'oklch(0.115 0.013 265)' }}>
      <EditorTopbar
