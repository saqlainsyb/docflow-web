...
// ─────────────────────────────────────────────────────────────────────────────
// Board role permission gating via board.my_board_role + useBoardPermissions.
//
// Board menu items are now conditionally rendered:
//   Rename        → owner + admin
//   Change visibility → owner only
//   Share link    → owner + admin  (Share button in topbar)
//   Delete        → owner only
//
// The resolved board role comes directly from BoardDetailResponse.my_board_role
// — the backend already computed it server-side including workspace override
// logic. No re-derivation from the members array here.
//
// Member avatar tooltips now show the board role (Owner / Admin / Editor)
// instead of the workspace role.
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3 shrink-0">
          {/* Member stack — tooltips show board role */}
              ...
              <div className="flex -space-x-2.5">
                {visibleMembers.map((member, i) => (
                  <motion.div
                    ...
                    // Tooltip shows name + board role (not workspace role)
...
export function BoardPage() {
  ...
  // Permission hook — uses the backend-resolved board role
  const perms = useBoardPermissions(board?.my_board_role);
